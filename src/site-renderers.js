  ...
  return `
    <aside class="sidebar">
      <div class="sidebar__section">
        ...
        <a class="sidebar__brand" href="${prefix}index.html">Design System</a>
        <p class="sidebar__description">Identidade, tokens, componentes e padrões para admin, front e futuros produtos.</p>
      </div>
      ${nav.map((group) => `
        <section class="sidebar__section">
          <h2 class="sidebar__title">${escapeHtml(group.title)}</h2>
          <div class="sidebar__links">
            ${group.items.map((item) => navLink(item, activeNav, prefix)).join("")}
          </div>
        </section>
      `).join("")}
    </aside>
  ...
  return `
    <div class="code-block">
      <div class="code-block__header">
        ...
        <button class="ghost-button" type="button" data-copy-code data-copy-value="${escapeHtml(code)}">Copiar</button>
      </div>
...
function previewDeviceRow(items) {
  return `<div class="device-row">${items.join("")}</div>`;
}

function doDont(doHtml, dontHtml) {
  return `<div class="do-dont"><div class="do-dont__good"><h3>DO</h3>${doHtml}</div><div class="do-dont__bad"><h3>DON'T</h3>${dontHtml}</div></div>`;
}

function callout(label, text) {
  return `<aside class="callout"><strong>${escapeHtml(label)}</strong><p>${escapeHtml(text)}</p></aside>`;
}
...
function hero(page, extraHtml = "") {
  return `
    <section class="hero">
      <div>
        <p class="eyebrow">${escapeHtml(page.category || "Getting started")}</p>
        <h1>${escapeHtml(page.title)}</h1>
        <p class="hero__description">${escapeHtml(page.description)}</p>
      </div>
...
function pageArticle(title, body, extra = "") {
  return `<article class="prose"><h2>${escapeHtml(title)}</h2>${body}${extra}</article>`;
}

function brandSwatches() {
  return `
    <div class="swatch-grid">
      <div class="swatch"><span>Canvas</span><strong>#090909</strong></div>
      <div class="swatch"><span>Surface</span><strong>#111214</strong></div>
      <div class="swatch"><span>Accent</span><strong>#D4FB00</strong></div>
      <div class="swatch"><span>Light Canvas</span><strong>#F5F0E8</strong></div>
    </div>
  `;
}
